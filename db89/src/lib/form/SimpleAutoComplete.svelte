<script>
     ...
     */
    export let searchFunction = false; //() => { return false } // было просто false
  ...
  <div
    class="{className ? className : ''} autocomplete {uniqueId}"
  ...
  <style>
    ...
    .autocomplete-list {
      ...
      max-height: calc(15 * (1rem + .2rem) + .2rem);
      user-select: none;
      white-space: normal;
      font: inherit;
    }
    ...
    .autocomplete-list-item.confirmed {
      ...
      color: #333;
      border-left: 3px solid #789fed;
      border-radius: 3px;
    }
    .autocomplete-list-item.selected {
      background-color: #777;
      color: #fff;
      border-left: 3px solid #2e69e2;
      border-radius: 3px;
    }
